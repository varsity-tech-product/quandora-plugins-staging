...
The contract `fixed_fields` must publish `initial_cash=100000`, `taker_fee_rate=0.0004`,
`maker_fee_rate=0.0002`, and `attribution=true`, all with `caller_supplied=false`. Treat them as
fixed server-owned assumptions: display them for confirmation but never send them as tool
arguments. Stop on any contract/schema or fixed-value mismatch; never send semantic,
...
choices and omit optional fields the user did not choose. If the user requests a different cash,
fee, or attribution value, explain the fixed contract and do not reinterpret it as a submit field.
...
weights, ranking, direction, dates, and rebalance choices, followed by the separately labelled
fixed capital, fees, and attribution assumptions, then obtain explicit confirmation.
