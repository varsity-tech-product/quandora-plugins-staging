...
Treat Task `category` and plugin `FACTOR_TYPE` as separate but strictly aligned fields. `category` is the closed product research classification retained by the Task/session lineage; `FACTOR_TYPE` is a specific, unique snake_case identifier for the implemented mechanism. For every custom factor, the thesis, formula, selected inputs, `FACTOR_NAME`, and `FACTOR_TYPE` must all semantically belong to the category the agent supplied to `fm_custom_sess`. Use a mechanism-specific value such as `funding_adjusted_trend_persistence` inside `Momentum`; do not use a bare category label such as `momentum`, and never infer or rewrite the category from `FACTOR_TYPE`, names, tags, or backtest performance.
