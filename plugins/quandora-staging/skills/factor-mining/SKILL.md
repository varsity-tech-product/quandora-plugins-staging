...
- For a custom idea: before choosing `category` and before `fm_custom_sess`, call `fm_list_tasks` exactly once unless its complete current response has already been obtained in the same conversation and workflow. Require exactly eight unique open public task references: every row must have a unique non-empty `task_id`, one of the eight non-`Other` canonical categories, and non-empty `core_question`, `primary_alpha_source`, `economic_principle`, `microstructure_logic`, `crypto_specific_mechanism`, `research_directions`, and `target_behavior` arrays. The set of their returned category values must equal exactly `Microstructure`, `Volatility`, `Imbalance`, `Order Flow`, `Auction`, `Momentum`, `Volume`, and `Liquidity`; every category must appear exactly once. Duplicate categories and any missing category are a backend/plugin contract mismatch and must stop the custom workflow before `fm_custom_sess`. If any expected task, category, or critical field is absent, malformed, duplicated, or inconsistent, stop before session creation and report a backend/plugin contract mismatch. Do not fall back to a static classification guess or stale model memory, and do not invent missing content.
