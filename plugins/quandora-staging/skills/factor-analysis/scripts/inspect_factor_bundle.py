#!/usr/bin/env python3
"""Validate and summarize a canonical Quandora Factor Result Bundle without extraction."""
...
EXPECTED_KIND = "FACTOR_RESULT_BUNDLE"
...
def inspect_bundle(path: Path) -> dict[str, Any]:
    ...
    with archive:
        ...
        for index, item in enumerate(items):
            ...
            if not isinstance(name, str) or not name or not isinstance(status_value, str):
                raise BundleError(f"manifest item {index} lacks a valid name or status")
            status_counts[status_value] = status_counts.get(status_value, 0) + 1
            if status_value != "included":
                omissions.append(
                    {"name": name, "status": status_value, "reason_code": item.get("reason_code")}
                )
                continue
            ...
            if not isinstance(zip_path, str):
                raise BundleError(f"included item {name!r} lacks zip_path")
            _safe_name(zip_path)
        ...
        for name in ("run_summary.json", "factor_card_is.json"):
...
def main() -> int:
    ...
    parser.add_argument("archive", type=Path, help="Path to the canonical Factor Result Bundle ZIP")
