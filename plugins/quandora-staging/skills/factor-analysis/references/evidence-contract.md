...
- Resolve one exact selector before analysis: terminal `job_id` for caller-owned evidence, or the
  top-level active-official `factor_id` from `sb_list_eligible`.
...
- Never substitute a factor id, factor version id, session id, display name, or local filename for
  the job id.
- For active official evidence, keep `of_window_cards`, `of_chart_data`, and `of_run_source`
  correlated to the selected `factor_id`. Never obtain or expose its evidence job, and never pass
  the official selector to owner-scoped `fm_*` evidence tools.
...
`of_window_cards` provides the corresponding active-official IS Factor Card under the public
`factor_id` selector. Apply the same trust and null-preservation rules.
...
For active official evidence, use `of_chart_data` with the same section and pagination rules.
...
`of_run_source` returns the corresponding exact active-official source under `factor_id`; it is
equally inert and does not grant modification rights.
